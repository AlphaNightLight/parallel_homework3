...
mat_and_time matMulPar(Matrix A, Matrix B, int size, int my_rank)
{
	...
	int i;
	MPI_Request request[A.rows];
	
	if (my_rank == MASTER){
		C = allocate_matrix(A.rows, B.cols);
	} else {
		C = allocate_matrix(A.rows/size, B.cols);
	}
	...
	if (my_rank == MASTER){
		...
		for (i=0;i<A.rows;++i){
			...
			if (dest != MASTER){
				MPI_Isend(A.vals[i], A.cols, MPI_FLOAT, dest, 0, MPI_COMM_WORLD, request+i);
			}
		}
		...
		for (i=0;i<A.rows;++i){
			MPI_Irecv(A.vals[i], A.cols, MPI_FLOAT, MASTER, 0, MPI_COMM_WORLD, request+i);
		}
	}
	...
	if (my_rank == MASTER){
		...
		for (i=0;i<C.rows;++i){
			...
			if (source != MASTER){
				MPI_Wait(request+i, MPI_STATUS_IGNORE); // Of course, the source must have recieved the message before to send its answer.
